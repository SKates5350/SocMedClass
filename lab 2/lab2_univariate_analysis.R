...
dataset[which(dataset$likes_count>500000), ] ## all posts with 500+ likes

# Something new: how can we combine statements to subset the data frame?

# a) to specify an AND condition:
dataset[which(dataset$likes_count>250000 & dataset$comments_count>20000), ] 
## all posts with 100+ likes AND 30+ comments

# b) to specify an OR condition:
dataset[which(dataset$type=="status" | dataset$comments_count>20000), ] 
...
hist(dataset$likes_count, breaks=100, 
